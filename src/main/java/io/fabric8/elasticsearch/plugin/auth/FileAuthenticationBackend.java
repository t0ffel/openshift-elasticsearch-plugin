...
public class FileAuthenticationBackend implements AuthenticationBackend, HTTPAuthenticator {
    ...
    @Override
    public AuthCredentials extractCredentials(RestRequest request, ThreadContext threadContext) throws ElasticsearchSecurityException {
    ...
    private Settings loadAuthFile() {
        ...
        if (now > lastModified) {
            try {
                ...
                mappings = Settings.builder().put(new YamlSettingsLoader(false).load(BytesReference.toBytes(ref))).build();
                lastModified = now;
